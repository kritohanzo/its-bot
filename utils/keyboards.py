...
def generate_back_to_menu_button():
    back_to_menu_builder = ReplyKeyboardBuilder()
    back_to_menu_builder.button(text='В главное меню')
    back_to_menu_builder.adjust(1)
    return back_to_menu_builder.as_markup(resize_keyboard=True)
...
def generate_keyboard_with_users(sender_telegram_id):
    ...
    return users_builder.as_markup(resize_keyboard=True)